...
{
    const string DetailCostLabelDefault = "Custo:";
    const string DetailCostLabelPassive = "Sem Custo";
    ...
    void ApplyDetailCost(SkillDataSO skill)
    {
        ...
        int maxIndex = _detailCostBadgeSprites != null && _detailCostBadgeSprites.Length > 0
            ? _detailCostBadgeSprites.Length - 1
            : 3;
        ...
        {
            _detailCostBadgeImage.sprite = ResolveDetailCostBadgeSprite(cost);
            _detailCostBadgeImage.enabled = true;
        }
    }

    void SetDetailCostLabel(string text)
    {
        if (_detailCostLabelText != null)
    ...
    {
        if (_detailCostBadgeSprites == null || _detailCostBadgeSprites.Length == 0) return null;
        int index = Mathf.Clamp(cost, 0, _detailCostBadgeSprites.Length - 1);
        return _detailCostBadgeSprites[index];
    }

    void ResolveDetailCostReferences()
    {
        if (_detailCostLabelText == null)
            _detailCostLabelText = FindDetailChildTmpText("CostLabel", "txt_CostLabel", "CustoLabel");
    ...
    TMP_Text FindDetailChildTmpText(params string[] names)
    {
        ...
        {
            if (text == null || text == _detailCostLabelText) continue;
            foreach (string name in names)
            {
                if (text.name.Equals(name, StringComparison.OrdinalIgnoreCase))
                    return text;
            }
        }
    ...
    Image FindDetailChildImage(params string[] names)
    {
    ...
    void BuildDetailCostRow(Transform parent)
    {
        var costRow = CreateUiObject("DetailCostRow", parent);
        StretchRect(costRow.GetComponent<RectTransform>(), new Vector2(0.32f, 0.15f), new Vector2(0.58f, 0.28f));

        var costLabel = CreateText(costRow.transform, "CostLabel", "Custo:", new Vector2(0f, 0.45f), new Vector2(0.42f, 1f), 13f);
        costLabel.alignment = TextAlignmentOptions.Left;

        var badgeObj = CreateUiObject("CostBadge", costRow.transform);
        var badgeRect = badgeObj.GetComponent<RectTransform>();
        StretchRect(badgeRect, new Vector2(0.46f, 0f), new Vector2(1f, 1f));
        var badgeImage = badgeObj.AddComponent<Image>();
        badgeImage.color = Color.white;

        _detailCostLabelText = costLabel;
        _detailCostBadgeImage = badgeImage;
    }
